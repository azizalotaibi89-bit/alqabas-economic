...
function TenderCard({ article, tab }) {
  ...
  // Prefer dedicated Arabic subject; fall back to title (ref code)
  const hasSubject = article.tenderSubject && article.tenderSubject.length > 5;
  const displayTitle = hasSubject ? article.tenderSubject : article.title;
  const showRef = hasSubject && article.tenderRef;
  ...
  return (
    ...
    >
      ...
      {/* Title = Arabic subject when available, otherwise ref code */}
      <h4
      ...
      {/* Reference code badge (shown only when we have a real Arabic title above) */}
      {showRef && (
        <div className="mt-1 mb-1" style={{ direction: 'ltr', textAlign: 'left' }}>
      ...
      {/* Ministry row */}
      {article.tenderMinistry && (
        <p className="text-xs text-gray-600 font-medium mt-1 mb-1 line-clamp-1"
           style={{ direction: 'rtl', textAlign: 'right' }}>
          🏛️ {article.tenderMinistry}
        </p>
      )}
